...
class Fanv2Accessory extends BaseAccessory {
  constructor(platform, homebridgeAccessory, deviceConfig) {
    ...
    this.speed_enum_count = this.getSpeedFunctionLevel("fan_speed_enum")
    this.speed_enum_coefficient = 100 / this.speed_enum_count
  ...
  refreshAccessoryServiceIfNeed(statusArr, isRefresh) {
    ...
    for (const statusMap of statusArr) {
      if (statusMap.code === 'switch' || statusMap.code === 'switch_fan') {
      ...
      if (statusMap.code === 'fan_speed_enum') {
        this.speedMap = statusMap
        const hbSpeed = parseInt(this.speedMap.value.substr(6, 1) * this.speed_enum_coefficient);
        this.normalAsync(Characteristic.RotationSpeed, hbSpeed, this.isRefresh)
      } else if (!this.speedMap && statusMap.code === 'fan_speed_percent') {
        this.speedMap = statusMap
      ...
      } else if (!this.speedMap && statusMap.code === 'fan_speed') {
        this.speedMap = statusMap;
  ...
  getSendParam(name, hbValue) {
    ...
    switch (name) {
      ...
      case Characteristic.RotationSpeed:
        ...
        } else if (this.speedMap.code === 'fan_speed_enum') {
          let level = Math.floor(hbValue / this.speed_enum_coefficient) + 1
          level = level > this.speed_enum_count ? this.speed_enum_count : level;
          speed = "level_" + level;
        } else {
