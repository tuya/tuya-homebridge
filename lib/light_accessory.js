...
class LightAccessory extends BaseAccessory {
  constructor(platform, homebridgeAccessory, deviceConfig) {
    ...
    //get Lightbulb function dp range
  ...
  getSendParam(name, value) {
    ...
    switch (name) {
      ...
      case Characteristic.Hue:
        ...
        saturation = Math.floor((this.function_dp_range.bright_range.max - this.function_dp_range.bright_range.min) * this.getCachedState(Characteristic.Saturation) / 100 + this.function_dp_range.bright_range.min);// value 0~100
        code = this.colourData.code;
        value = {
          "h": value,
          "s": saturation,
