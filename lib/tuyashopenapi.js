...
class TuyaSHOpenAPI {
  ...
  constructor(endpoint, accessId, accessKey, username, password, countryCode, appSchema, lang = 'en') {
  ...
  async getDevices() {
    ...
    let devices = [];
    for (let i = 0; i < res.result.devices.length; i++) {
      devices.push(Object.assign({}, res.result.devices[i], devicesFunctions.find((j) => j.devices[i] == res.result.devices[i].id)))
    }
